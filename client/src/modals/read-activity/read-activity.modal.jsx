...
const ReadActivityModal = ({open, closeModal, activity, onlyClose, reportId, reportAssignment, reportLevel}) => {
    ...
    useEffect(() => {
        ...
        if(activity.readCommits) {
            if(activity.readCommits.length > 0) {
                setCommitsLength(activity.readCommits.length);
            }
            setCommits(activity.readCommits);
        }else{
            setCommits([]);
        }
    }, [activity]);
    ...
    const uploadImageReport = async (file) => {
        if(file) {
            ...
            setCommits([]);
            let totalImagenes = 0
            if(!activity.ejecucionImages) {
                activity.ejecucionImages = totalImagenes
            }else{
                totalImagenes = activity.ejecucionImages
            };
            let obj = {
                user: localStorage.getItem('_id'),
                userName: `${localStorage.getItem('name')} ${localStorage.getItem('lastName')}`,
            ...
            if(navigator.onLine) {
                ...
                let imageUploaded = await uploadImage(file, 'reports', `${reportId}/image_report_obs_${activity.strpmdesc}_${activity.task}_${arr.length}`);
                    ...
                    <div>
                        ...
                        <div style={{borderRadius: 20, zIndex: 1, backgroundColor: '#fff', borderStyle: 'solid', borderWidth: 0.5, borderColor: '#BDBDBD', height: '20vh', padding: 0}}>
                            <div id='commits' style={{width: '100%', borderBottomStyle: 'solid', borderBottomColor: '#BDBDBD', borderBottomWidth: 0.5, height: '14vh', maxHeight: '14vh', overflowY: 'auto'}}>
                                {
                                    (commits.length > 0) && commits.reverse().map((element, n) => {
                                        return(
                                            <div key={n} id={`message_${n+1}`} style={{width: '100%', display: 'inline-block', paddingLeft: 20, paddingRight: 20}}>
                                                <div style={{padding: 10, margin: 5, maxWidth: '80%', backgroundColor: '#F9F9F9', borderRadius: 20}}>
