const initDb = () => {
    ...
    const conexion = indexedDb.open('Images',1)
    ...
    return new Promise(resolve => {
        ...
        conexion.onupgradeneeded = (e) =>{
            db = e.target.result
            const coleccionObjetos = db.createObjectStore('SinglesImages',{
                keyPath: 'name'
            })
...
const agregar = (data, database) => {
    const trasaccion = database.transaction(['SinglesImages'],'readwrite')
    const coleccionObjetos = trasaccion.objectStore('SinglesImages')
...
const obtener = (clave, database) =>{
    return new Promise(resolve => {
        const trasaccion = database.transaction(['SinglesImages'],'readonly')
        const coleccionObjetos = trasaccion.objectStore('SinglesImages')
